"""CLIP ViT-L/14 batch embedder — parallel to siglip_batch.py per Decision 23.

Same shape as siglip_batch.py:
  - Reads frame metadata from `{BUCKET_URI}/{dataset_prefix}/metadata/*.parquet` via DuckDB
  - Pulls thumbnails via fsspec
  - Writes float16 embeddings to a SEPARATE LanceDB table (so we can A/B vs SigLIP)
  - Resumable

Model: openai/clip-vit-large-patch14 (512-dim image embeddings).
LanceDB table naming: <dataset>_clip (e.g. waymo_clip, bdd100k_clip) to separate from SigLIP.
"""
...
@app.command()
def main(
    table: str = typer.Option(..., help="Dataset name (waymo / bdd100k / ...). LanceDB table = <table>_clip."),
    ...
    """Embed frames into LanceDB table <table>_clip with CLIP ViT-L/14."""
    load_dotenv()
    _lazy_imports()
    ...
    lance_table = f"{table}_clip"

    console.rule(f"[bold]CLIP embed → {lance_table}")
    console.print(f"  model:        {model_name}")
    console.print(f"  device:       {device}")
    console.print(f"  metadata_uri: {metadata_uri}")
    console.print(f"  lance:        {lance_dir}/{lance_table}")
    ...
    embed_dim = model.config.projection_dim  # CLIP image features projected to projection_dim (768 for L/14)
    ...
    console.print(f"[green]done — committed {committed:,} new CLIP embeddings to {lance_table}[/green]")
